...
function MailJob (id, from, to, subject, template, data, language) {
    this._id = id || null;
...
MailJob.fromPersistenceJob = function(persistenceJob) {
    var payload = persistenceJob.payload;
    var id = persistenceJob._id;

    return new MailJob(id, payload.from, payload.to, payload.subject, payload.template, payload.data, payload.language);
};
